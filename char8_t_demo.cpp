...
void char8_t_usage_demo()
{
	...
	// In this demo, we mostly use narrow string for printing with std::cout. 
	// However, because u8 strings need to be converted to wide char, we use wide strings and wcout for this example.
	...
	std::wcout << L"hash(u8" << std::quoted(my_wstr) << ") = 0x" << std::hex << str_hash << '\n';
	std::wcout << L"hash(L" << std::quoted(my_wstr) << ") = 0x" << std::hex << str_whash << '\n';
}
...
void char8_t_path_demo()
{
	...
	std::u8string my_file_path = u8R"("C:\Windows\System32\msvcp140.dll")";
